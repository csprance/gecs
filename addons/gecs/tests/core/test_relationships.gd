...
func test_component_target_with_weak_matching():
	# Test weak matching with component targets - should match by component type regardless of data
	...
	# Strong matching should find exact poison levels only
	var poison_1_entities = Array(ECS.world.query.with_relationship([Relationship.new(status_effect_marker, poison_level_1)]).execute())
	var poison_5_entities = Array(ECS.world.query.with_relationship([Relationship.new(status_effect_marker, poison_level_5)]).execute())
	...
	# Test weak matching on individual entities - should find any poison level of same type
	var bob_has_any_poison = e_bob.has_relationship(Relationship.new(status_effect_marker, C_Eats.new(999)), true)
	var alice_has_any_poison = e_alice.has_relationship(Relationship.new(status_effect_marker, C_Eats.new(999)), true)
	var heather_has_any_poison = e_heather.has_relationship(Relationship.new(status_effect_marker, C_Eats.new(999)), true)
	...
	# Verify we can retrieve the actual poison levels using weak matching
	var bob_poison_rel = e_bob.get_relationship(Relationship.new(status_effect_marker, C_Eats.new(999)), true, true)
	var alice_poison_rel = e_alice.get_relationship(Relationship.new(status_effect_marker, C_Eats.new(999)), true, true)
	var heather_poison_rel = e_heather.get_relationship(Relationship.new(status_effect_marker, C_Eats.new(999)), true, true)
...
func test_component_archetype_target_matching():
	...
	var bob_strength_rel = e_bob.get_relationship(Relationship.new(has_buff_marker, C_Likes.new(999)), true, true)
	var heather_strength_rel = e_heather.get_relationship(Relationship.new(has_buff_marker, C_Likes.new(999)), true, true)
...
func test_multiple_component_targets_same_relationship():
	...
	var existing_alice_attacking = e_alice.get_relationships(Relationship.new(C_IsAttacking.new(), null), true)
...
func test_broad_query_with_drill_down_filtering():
	...
	# Step 2: Drill down - find entities with ANY fire damage (weak matching)
	var fire_damaged_entities = []
	for entity in all_damaged_entities:
		# Use weak matching to find any fire damage type regardless of amount
	...
	# Step 3: Drill down further - find entities with HIGH fire damage (strong matching + manual filter)
	var high_fire_damage_entities = []
	for entity in fire_damaged_entities:
		# Get the actual fire damage relationship using weak matching
		var fire_rel = entity.get_relationship(Relationship.new(C_Damaged.new(), C_FireDamage.new(999)), true, true)
	...
	for entity in all_damaged_entities:
		var has_fire = entity.has_relationship(Relationship.new(C_Damaged.new(), C_FireDamage.new(999)), true)
		var has_poison = entity.has_relationship(Relationship.new(C_Damaged.new(), C_PoisonDamage.new(999)), true)
