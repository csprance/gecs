...
#
	...
	#entity3.add_component(health_component)
#
...
#
	...
	#var result = world.query([Velocity])
	#assert_eq(result.size(), 3, "All entities with Velocity should be returned.")
	#result = world.query([Velocity, Transform])
	#assert_eq(result.size(), 2, "Entities with both Velocity and Transform should be returned.")
