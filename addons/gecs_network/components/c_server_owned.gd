class_name C_ServerOwned
extends Component

## Marker component indicating this entity is authoritative on the server.
##
## Automatically assigned by NetworkSync when C_NetworkIdentity.peer_id is 0 or 1.
##
## Added to:
...
##       return q.with_all([C_EnemyAI, C_ServerOwned])
##
## 2. Skip server-owned on clients:
##   func query():
##       return q.with_all([C_EnemyAI]).with_none([C_ServerOwned])
##
